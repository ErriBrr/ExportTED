...
namespace TEDExporter
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IEnumerable<ImportLeanKit> resFSS = nouveauxTed.Where(x => x.SousSysteme == "Prestations FSS").Select(x => TedToLeaKitMapper.GetDTOForTED(x));
            if (resFSS.Any())
            ...
            IEnumerable<ImportLeanKit> resTauri = nouveauxTed.Where(x => x.SousSysteme.StartsWith("Actuaria")).Select(x => TedToLeaKitMapper.GetDTOForTED(x));
            if (resTauri.Any())
                ExportWriter(filePath, "Tauri", resTauri);
            IEnumerable<ImportLeanKit> resSCommun = nouveauxTed.Where(x => x.SousSysteme == "ServicesCommuns").Select(x => TedToLeaKitMapper.GetDTOForTED(x));
            if (resSCommun.Any())
